...
CONN_STR = "host='localhost',user='root',passwd='1902',database='sys'"
global admin_name
admin_name = "yogesh"
...
def gst_submit():
	...
	conn =  mysql.connector.connect(host='localhost',user='root',passwd='1902',database='sys')
	curr = conn.cursor()
...
def print_delivery_op():
	try:
		...
		try:
			conn =  mysql.connector.connect(host='localhost',user='operator1',passwd='operator1',database='sys')
			curr = conn.cursor()
			sql_create = f"""SELECT * FROM second_shop WHERE entry_code={enc} """
			curr.execute(sql_create)
			dataa = curr.fetchall()
...
def operator_log():
	...
	try:
		# create connection
		global con
		con =  mysql.connector.connect(host='localhost',user='operator1',passwd='operator1',database='sys')
